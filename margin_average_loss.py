...
def margin_classifiers_perf(d=1000,n=100,approx_tau=8, SNR=10, n_test=1e4, s=None, random_flip_prob=0, l1=False):
    ...
    # l2 margin
    clf = svm.LinearSVC(loss='hinge', fit_intercept=False, C=100)
    clf.fit(xs, ys)
    ...
    if l1 is False:
        ...
        clf = svm.LinearSVC(penalty='l1', fit_intercept=False, dual=False, C=100)
        clf.fit(xs, ys)
        wmm = -torch.Tensor(clf.coef_.flatten())
        ...
        for a in a_vals:
            ...
            while w.grad is None or torch.norm(w.grad) > 1e-3: # > 1e-5:
                optim.zero_grad()
                l = loss_average_margin(w, b, z1s, z2s, n1, n2) + torch.norm(w, p=1)
                l.backward()
                optim.step()
                #scheduler.step()
                steps += 1
