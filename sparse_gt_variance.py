...
def margin_classifiers_perf(d=1000,n=100,approx_tau=8, SNR=10, n_test=1e4, s=None, random_flip_prob=0, l1=True, l2=True):
    ...
    for a in a_vals:
        ...
        #scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=int(1e4), gamma=0.997)
...
def aspect_ratio_l1(d, n, change_d, n_runs=10, sigma=0, s=1):
    ...
    for run in range(runs):
        ...
        for t in approx_ar:
            ...
            err_mm_l1, errs_avm_poly_l1, err_train_mm_l1, errs_train_avm_poly_l1, wmm, wam = margin_classifiers_perf(d=d,n=n,approx_tau=1, SNR=10, n_test=1e4, s=s, l1=True, random_flip_prob=sigma)
            perf_mm_l1.append(err_mm_l1)
            perfs_l1.append(errs_avm_poly_l1)
            wmms.append(wmm)
            wams.append(wam)

            if err_train_mm_l1 + errs_train_avm_poly_l1 > 0:
        ...
        run_data.append({"run": run, "ars": ars, "a_vals": [0.],
                            "perfs_l1": perfs_l1, "perf_mm_l1": perf_mm_l1, 'wmm': wmms, 'wam': wams})
